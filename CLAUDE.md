...
| `/seek <position>` | seek.ts | перемотка текущего трека, абсолютно или относительно (см. "Seek" ниже) |
...
- `scripts/cookies-to-storagestate.mjs` — конвертер `cookies.txt` (Netscape) → `storageState.json` (Playwright). Чистый Node, без зависимостей — разовый шаг при (пере)настройке `cookie-refresher/`.

### `cookie-refresher/` — отдельный сервис, не часть бота

Продлевает YouTube-сессию по расписанию вместо ручного переэкспорта кук
(см. "Автопродление сессии" в README.md — там же порядок разовой настройки
и cron-команда). Ключевые решения:

- **Отдельный Docker-образ и `docker-compose.yml`-сервис**, не встроено в
  основной бот — headless-браузер (даже урезанный `chromium-headless-shell`)
  сюда не тащим в 512MB-лимит бота, и не хотим тянуть Playwright в
  зависимости продакшн-рантайма ради опциональной фичи.
- **`profiles: ["cookie-refresh"]`** в compose — НЕ стартует через обычный
  `docker compose up -d`, только явным `docker compose --profile
  cookie-refresh run --rm cookie-refresher` (это и делает cron на хосте).
  Между запусками браузер не висит вообще — ноль постоянного потребления.
- **`refresh.mjs` перед перезаписью проверяет**, что сессия всё ещё
  залогинена (наличие кук `SID`/`SAPISID`/`LOGIN_INFO` среди актуальных) —
  если нет, НИЧЕГО не трогает и падает с ненулевым кодом (видно в
  `cookie-refresh.log`). Это специально: тихая перезапись рабочего файла
  протухшим/разлогиненным — именно то, что уже один раз незаметно сломало
  возрастные видео (см. инцидент про `:ro` выше). Молчаливая деградация
  хуже явного "перестало работать, нужен новый экспорт".
- Тот же `./secrets/` на хосте, что и у основного бота: `cookie-refresher`
  монтирует его read-write (сам процесс контролируемый, пишет целиком
  корректный файл за раз — не то же самое, что дать это делать самому
  yt-dlp), бот — по-прежнему `:ro`.
- **Не панацея.** Google может в любой момент потребовать повторной
  верификации аккаунта (особенно учитывая, что заходы идут с
  датацентрового IP, а не оттуда, откуда аккаунт обычно логинится) —
  тогда `refresh.mjs` начнёт молча (для пользователя бота, не для логов)
  отказывать, и понадобится новый ручной экспорт с нуля.
...
### Позиция воспроизведения и Seek (`GuildPlayer.getPositionMs()` / `/seek`)

Позиция трека нигде не декодируется из аудио — считается по времени:
`playbackBaseMs` (накопленная позиция на момент последнего play/seek/pause)
+ `Date.now() - playbackStartedAt`, если сейчас реально играет
(`playbackStartedAt = null` на паузе — тогда просто отдаём застывший
`playbackBaseMs`, отсчёт не тикает). `pause()`/`resume()` обязаны
поддерживать эту пару в консистентном состоянии — если добавляешь ещё
один способ поставить на паузу/снять с паузы в обход этих методов, не
забудь про `playbackBaseMs`/`playbackStartedAt`, иначе прогресс-бар и
`/seek +N` (relative) начнут врать.

`/seek` пересоздаёт ffmpeg-поток целиком (`AudioPipeline.createPlayback(track, startSeconds)`
с `-ss` ДО `-i` — input seek, ffmpeg сам делает Range-запрос к googlevideo
URL и прыгает к ближайшему кадру, не перекачивая всё до нужной точки) и
подменяет `AudioResource` через `audioPlayer.play()` поверх уже играющего —
это НЕ проходит через `AudioPlayerStatus.Idle` (переход Playing→Playing
напрямую), поэтому `handleTrackFinished()`/автоплей/history на seek не
триггерятся — и это правильно, трек не "закончился", он просто продолжился
с другого места. `seek()` создаёт новый `PlaybackHandle` ДО того, как
убить старый (`currentHandle`) — если резолв/ffmpeg упадёт, текущее
воспроизведение остаётся нетронутым, а не обрывается в тишину. Плюс
race-guard: если за время ожидания сети трек сменился (`skip`/`stop`) —
новый handle тихо уничтожается, ничего не подменяется.
...
- **`YTDLP_COOKIES_FILE` volume ДОЛЖЕН быть `:ro` — это выяснилось не с первого раза, см. цепочку инцидентов.**
  1. Смонтировали куки read-only "для безопасности" — yt-dlp успешно искал/резолвил (видно только в stdout), но в конце пытался дозаписать обновлённые session-куки в тот же файл, падал на этом с `OSError: Read-only file system` и ненулевым кодом выхода, а `run()` на тот момент выбрасывал ЛЮБОЙ ненулевой код выхода как полный провал — валидные данные из stdout терялись. Починили `run()`: теперь не ревертит результат, если в stdout всё же есть данные, а код выхода ненулевой не из-за таймаута — только логирует warning. Это была правильная и достаточная часть фикса.
  2. ...но заодно (ошибочно) сделали volume read-write, решив, что раз yt-dlp "хочет" писать — пусть пишет. Это оказалось хуже: yt-dlp при каждом сохранении cookie jar **молча выбрасывает сессионные куки** (без даты истечения) — а именно такими обычно экспортируются ключевые куки авторизации (`SID`/`HSID`/`SSID`/`APISID`/`SAPISID`/`LOGIN_INFO`). С каждым реальным вызовом (`resolveStreamUrl`) файл понемногу "усыхал" (86KB → 3KB → 1.6KB за несколько запусков) — и через десяток вызовов аккаунт переставал распознаваться как залогиненный вообще, возрастные видео начинали падать с `Sign in to confirm your age` ДАЖЕ С КУКАМИ. Никакой видимой ошибки при этом не было — деградация тихая и необратимая (потерянное не восстановить, кроме как новым экспортом).
  3. Итоговый фикс: volume снова `:ro` (см. `docker-compose.yml`) — полностью блокирует запись на уровне ФС, yt-dlp даже не пытается тронуть файл на диске. Ошибка "не смог сохранить куки" по-прежнему возникает у yt-dlp, но безвредна благодаря фиксу из пункта 1 — данные всё равно доходят до нас через stdout.

  **Урок:** если "чинишь" ошибку записи внешнего процесса — сначала проверь, а нужна ли эта запись вообще получателю данных (нам — не нужна, `run()` и так справляется). Разрешать запись "чтобы не ругался" может тихо испортить данные, которые процесс же и не должен был трогать.
- **С кукам нужен `--js-runtimes node`, иначе падает вообще всё, не только возрастные видео.** Реальный инцидент: после починки прав на cookies.txt всё равно падало на КАЖДОМ видео (даже полностью безобидных, без возрастных ограничений) с `ERROR: The page needs to be reloaded`. Причина — YouTube требует решать JS-челлендж подписи формата для запросов с активной сессией (кукам), а yt-dlp без `--js-runtimes node` не находит JS-рантайм для этого (даже когда node физически есть и в PATH — нужно явно указать флагом, авто-детект в `yt_dlp_ejs` его не подхватывает сам). Без кук эта проверка не запускается вообще, поэтому анонимные запросы всегда работали. Смена cookies.txt на другой (даже более чистый) файл ничего не меняла — дело было не в конкретных куках. Флаг добавлен в `withDefaultArgs()` в `ytdlp.ts` безусловно (не только когда заданы куки) — вреда анонимным запросам не даёт. Если после апдейта yt-dlp снова начнёт падать с этой ошибкой — сначала гони `yt-dlp ... -v` руками и смотри секцию `[pot]`/`[jsc]` в debug-выводе, не спеши грешить на куки.
- **Куки нужны ТОЛЬКО в `resolveStreamUrl` — не в поиске/автоплее.** Реальный инцидент: после починки `--js-runtimes node` куки стали подставляться в КАЖДЫЙ вызов yt-dlp безусловно (включая `searchYoutube`/`getRelatedTracks`) — и `/play` с explicit-контентом ("Uncensored", "Explicit" в названии) стал тихо отвечать "ничего не найдено": `yt-dlp` завершался с `EXIT 0`, но **без единой строки** в stdout — то есть не ошибка, а именно пустая выдача. Тот же запрос без `--cookies` находил все 5 видео. Причина — YouTube Restricted Mode/SafeSearch на аккаунте, привязанном к кукам: авторизованная сессия фильтрует такой контент из поисковой выдачи, анонимная — нет. Фикс: `run()`/`withDefaultArgs()` принимают `useCookies` (по умолчанию `false`), куки явно передаются только в `resolveStreamUrl` (единственное место, где реально нужен обход возрастного ограничения для получения playable-формата) — `searchYoutube`, `resolveFromUrl`, `getRelatedTracks` остаются анонимными. Урок: `EXIT 0` + пустой stdout — это не "ничего не найдено по сути", а тоже симптом, который стоит проверять руками до вывода "это нормально".
